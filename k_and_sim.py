...
if __name__ == '__main__':
    ...
    classes = (KNNBasic, KNNWithMeans, KNNBaseline)
    ...
    kf = KFold(n_splits=2, random_state=0)  # folds will be the same for all algorithms.

    table = []
    for klass in classes:
        for k in range(40, 941, 100):
            for simName in ['msd', 'cosine', 'pearson', 'pearson_baseline']:
                for userBased in [True, False]:
                    mean_dict = defaultdict(list)
                    sim_options = {'name': simName, 'user_based': userBased}
                    klass_run = klass(sim_options=sim_options)
                    ...
                    [mean_precision, mean_recall, mean_rmse, mean_mae, mean_cv_time] = [
                        ...
                        (sum(mean_dict['recall']) / len(mean_dict['recall'])),
                        (sum(mean_dict['rmse']) / len(mean_dict['rmse'])),
                        (sum(mean_dict['mae']) / len(mean_dict['mae'])),
                        str(datetime.timedelta(seconds=int((sum(mean_dict['time']) / len(mean_dict['time'])))))]

                    new_line = [klass.__name__, simName, userBased, k, mean_precision, mean_recall, mean_rmse, mean_mae,
                                mean_cv_time]
